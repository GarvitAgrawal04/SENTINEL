...
| Repo | Relationship |
|------|-------------|
| bullmq | Alternative — BullMQ for Redis-based; Agenda for MongoDB/Postgres/Redis-backed scheduling |
| node-cron | Alternative — node-cron for in-process cron; Agenda for distributed job scheduling |
| trigger.dev | Alternative — Trigger.dev for managed; Agenda for self-hosted |
| inngest | Alternative — Inngest for event-driven; Agenda for scheduled jobs |
| prisma / drizzle-orm | Complement — ORM for app data; Agenda manages its own job storage |

## Trigger Conditions
Route here when: need distributed job scheduling, MongoDB-backed job queues, recurring/scheduled jobs with persistence, job debouncing, pluggable storage backend for schedulers.

## Output Contract
Delivers: Distributed job scheduling with pluggable backends (MongoDB, PostgreSQL, Redis) and persistent logging.
